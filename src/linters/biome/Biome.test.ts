...
describe('Biome', () => {
  ...
  describe('with files containing special characters', () => {
    ...
    test.each([
      ['spaces', '/path with spaces/biome.json'],
      ['quotes', '/path"with"quotes/biome.json'],
      ['special chars', '/path;with&special|chars/biome.json'],
  ...
  describe('linter.lint with artifact files', () => {
    const artifactsDir = join(process.cwd(), 'test', 'artifacts', 'biome')
    const configPath = artifactsDir
  ...
  vi.doMock('util', () => ({
    promisify:
      ...
      async (_cmd: string, _args: string[], options?: { shell?: boolean }) => {
        ...
        return { stdout: '{"diagnostics":[]}', stderr: '' }
      },
  }))

  // Re-import Biome after mocking
  const { Biome: MockedBiome } = await import('./Biome')
  const mockedLinter = new MockedBiome()
